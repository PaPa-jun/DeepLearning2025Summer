...
def rnn_experiment(
    ...
    train_texts, eval_texts, train_labels, eval_labels = train_test_split(
        train_texts, train_labels, test_size=0.22, random_state=42
    )
...
def main():
    ...
    parser.add_argument(
        ...
        required=True,
        help="Type of model to use (RNN or Attention).",
    )
    ...
    parser.add_argument(
        ...
        default="word",
        choices=["word", "char"],
        help="Tokenizer mode (word or char).",
    )
    parser.add_argument(
        "--min_freq", type=int, default=10, help="Minimum frequency for tokenization."
    )
    parser.add_argument(
        "--special_tokens",
        type=list,
        default=["<pad>"],
        help="List of special tokens for the tokenizer.",
    )
    parser.add_argument(
        "--device",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=256,
        help="Maximum length of encoded sequences.",
    )
    parser.add_argument(
        "--embedding_dim", type=int, default=128, help="Dimension of word embeddings."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.3,
        help="Dropout rate for the Attention model.",
    )
    parser.add_argument(
        "--epochs", type=int, default=10, help="Number of training epochs."
    )
    parser.add_argument(
        "--hidden_size", type=int, default=64, help="Hidden size of the model layers."
    )
    parser.add_argument(
        "--output_size",
        type=int,
        default=2,
        help="Output size of the model (e.g., 2 for binary classification).",
    )
    parser.add_argument(
        "--num_heads",
        type=int,
        default=4,
        help="Number of attention heads (only for Attention model).",
    )
    parser.add_argument(
        "--use_bias",
        action="store_true",
        help="Whether to use bias in the Attention model.",
    )
    parser.add_argument(
        "--rotary",
        action="store_true",
        help="Whether to use rotary embeddings in the Attention model.",
    )
    ...
    tokenizer = Tokenizer(
        texts, args.tokenizer_mode, args.min_freq, special_tokens=args.special_tokens
    )
